...
class UserController extends GetxController {
  ...
  RxList<EmployeePropertyModel> employeePropertyModelList =
      <EmployeePropertyModel>[].obs;
  ...
  fetchAllUserRoles() async {
    ...
    try {
      ...
      return userRoleList
          .assignAll(data.map((json) => UserRoleModel.fromJson(json)).toList());
    } catch (error) {
      isUserListLoading(false);
  ...
  Future<void> getUserOrganizationData() async {
    try {
      final response = await AppConfig()
          ...
          .from('organisations')
          .select()
          .eq('id', userStorage.read('OrganizationId'))
          .execute();
  ...
  Future<void> getUserProfileData() async {
    try {
      await AppConfig()
        ...
        print(
            'MY SPECIFIC Profile User RESPONSE IS ${userValue.data[0]['first_name']}');
        print(
            'MY SPECIFIC Profile User  ID IS ${userValue.data[0]['user_id']}');
        userFirstname.value = userValue.data[0]['first_name'];
        await userStorage
            .write('userFirstname', userValue.data[0]['first_name'])
            .then((value) async {
          await userStorage
              .write('userProfileId', userValue.data[0]['id'])
              .then((value) async {
            await userStorage
                .write('OrganizationId', userValue.data[0]['organisation_id'])
                .then((value) async {
              await userStorage
                  .write('roleId', userValue.data[0]['role_id'])
                  .then((value) async {
  ...
  Future<void> createUserWithEmail(
    ...
    try {
      final AuthResponse response =
          await AppConfig().supaBaseClient.auth.signUp(
                password: password,
                email: email,
              );
  ...
  Future<void> loginEmailUser(String email, String password) async {
    ...
    try {
      final AuthResponse response =
          await AppConfig().supaBaseClient.auth.signInWithPassword(
                email: email,
                password: password,
              );
  ...
  signUpUser(String email, String password, String firstName, String lastName,
      String businessName, String description, String phone) async {
    UserModel userModel = UserModel(email: email, password: password);

    final AuthResponse res = await AppConfig()
        .supaBaseClient
        .auth
        .signUp(
    ...
    try {
      if (res.user != null) {
        ...
        Fluttertoast.showToast(
            msg: 'Error Signing Up', gravity: ToastGravity.TOP);
      }
    } catch (e) {}
  }
  ...
  Future<void> adminCreateUser(
    ...
    try {
      ...
      print(response.user!.id);
      adminCreateUserProfile(response.user!.id.toString(), firstName, lastName,
          role, phone, email);
    } catch (error) {
  ...
  addPropertyToEmployee(int propertyId, int roleId, String userId) async {
    ...
    try {
      final response =
          await AppConfig().supaBaseClient.from('employee_properties').insert([
        {
          "user_id": userId,
          "property_id": propertyId,
          "role_id": roleId,
