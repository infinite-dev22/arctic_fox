...
class TenantDetailsScreenLayout extends StatefulWidget {
  ...
  @override
  State<TenantDetailsScreenLayout> createState() => _TenantDetailsScreenLayoutState();
}

class _TenantDetailsScreenLayoutState extends State<TenantDetailsScreenLayout> with TickerProviderStateMixin{
  ...
  builder: (context, state) {
    if (state.status == TenantStatus.initial) {
      context.read<TenantBloc>().add(LoadSingleTenantEvent(widget.tenantId));
    }
    if (state.status == TenantStatus.loadingDetails) {
      ...
      return Column(
        ...
        children: [
          ...
          Center(child: Text(state.tenantModel!.clientTypeId! == 1 ? '${state.tenantModel!.tenantProfiles!.first.firstName}' ' ${state.tenantModel!.tenantProfiles!.first.lastName}'
              : state.tenantModel!.tenantProfiles!.first.companyName , style: AppTheme.appTitle5,)),
          SizedBox(height: 1.h,),
          Center(
            child: Card(
          ...
          Expanded(
            child: TabBarView(
                controller: tabController,
                children: [
                  TenantDetailsTab(tenantModel: state.tenantModel!, tenantController: widget.tenantController,),
    ...
    return Column(
        ...
        children: [
          ...
          Center(child: Text('${state.tenantModel!.tenantProfiles!.first.firstName}' ' ${state.tenantModel!.tenantProfiles!.first.lastName}', style: AppTheme.appTitle5,)),
          SizedBox(height: 1.h,),
          Center(
            child: Card(
              shape: RoundedRectangleBorder(
